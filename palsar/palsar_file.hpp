...
public:
    ...
    [[nodiscard]] bool IsOpen() const { return size_ > 0; }
...
public:
    ...
    [[nodiscard]] bool IsOpen() const { return file_size_ > 0; }

    [[nodiscard]] size_t Size() const { return file_size_; }

    // 720 is SAR Data File Descriptor size before Signal Data Records
    [[nodiscard]] const char* SignalData() { return data_.get() + 720; }
    [[nodiscard]] size_t SignalSize() const { return file_size_ - 720; }
    [[nodiscard]] const char* SignalDataRow(int row_idx, int record_size) {
        return data_.get() + 720 + (row_idx * record_size);
    }
...
public:
    ...
    void ReadAllData(SARImage& img, SARMetadata& metadata);
